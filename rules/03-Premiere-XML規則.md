...
**這條規則只在Premiere上驗證過**，不要預設每個使用者都用Premiere——[`SKILL.md`](../SKILL.md) 第10步會先問使用者實際用哪套剪接軟體。使用者用Final Cut／DaVinci／其他軟體時，這份schema沒有實測過，要老實講清楚未驗證，不要當成一定能用；保底版（ffmpeg直接切好的小檔案）或本文件最後一條提到的CMX3600 EDL，才是不挑軟體、比較保證能用的備案。
